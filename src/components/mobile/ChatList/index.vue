<template>
  <div class="head">
    ...
    <div v-for="(item, ind) in unreadGroup" :key="ind">
      <van-cell
        :icon="item.avatar"
        :title="item.groupName"
        :value="item.count"
        @click="toPathGroup(item.id)"
        is-link
        style="background-color:rgb(250,250,250);text-align:left;border:solid 0.02rem #ddd; border-top:none; border-left:none; border-right:none"/>
    </div>
  </div>
...
export default {
  ...
  methods:{
    ...
    toPathGroup(id){
      this.$router.push({name: "groupChat", params: {groupNum: id}});
    },
    init(){
    ...
    parpareData(){
      ...
      for (let i = 0; i < this.unreadLeaveFriend.length; i++) {
        // console.log("parpareData未读信息数量", this.unreadLeaveFriend.length);
        //用户离线时的未读消息展示逻辑
        if (this.unreadLeaveFriend[i].fromUser == undefined) {
          // console.log(i, "进来了");
          if (this.groupUnreadNumber[this.unreadLeaveFriend[i].groupDO.id] > 0) {
            // console.log("parpareData未读消息进来了吗");
            let t = {
              id: this.unreadLeaveFriend[i].groupDO.groupNum,
              groupName: this.unreadLeaveFriend[i].groupDO.groupName,
